...
class _SempoaChallengeScreenState extends State<SempoaChallengeScreen> {
  ...
  void _showFeedback(bool correct) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          correct ? 'BENAR! +10 Poin, +2s' : 'SALAH! Streak putus.',
  ...
  void _endChallenge() {
    _saveAndStopChallenge(
      showResultDialog: true,
      title: 'Waktu Habis!',
      message: 'Skor Akhir Anda: $_currentScore',
    );
  }

  void _saveAndStopChallenge({
    ...
    DataService(context).saveSempoaProgress(
      newScore: _currentScore,
      newLevel: _currentLevel,
      newStreak: _bestStreak,
    );
    if (showResultDialog) {
      showDialog(
        context: context,
        builder: (_) => AlertDialog(
          title: Text(title ?? 'Selesai'),
          content: Text(message ?? 'Skor: $_currentScore'),
        ),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final bool isCriticalTime =
        _isChallengeActive && _remainingSeconds <= 10 && _remainingSeconds > 0;
        ...
        if (_isChallengeActive) {
          final confirm = await showDialog<bool>(
            ...
            builder: (_) => AlertDialog(
              ...
              content: const Text(
                'Jika keluar sekarang, waktu akan berhenti dan progres disimpan otomatis.',
              ),
              actions: [
                TextButton(
        ...
        children: [
          ...
          Expanded(
            child: ElevatedButton.icon(
              ...
              icon: Icon(
                _isChallengeActive ? Icons.check : Icons.flash_on,
                color: Colors.white,
                size: 18,
              ),
              label: FittedBox(
  ...
  Widget _buildStatCards() {
    final accuracy = _totalAttempts == 0
        ? 0
        : ((_accuracyCount / _totalAttempts) * 100).round();
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                '$progress / 100 poin ke level berikutnya',
      ...
      children: [
        InkWell(
          ...
          child: Container(
            ...
            child: Icon(
              Icons.circle,
              color: active ? const Color(0xFF7A5CFF) : Colors.grey,
              size: size * 0.45,
            ),
          ),
        ),
        ...
        Text(
          ...
          style: TextStyle(
            color: Colors.grey[700],
            fontSize: (size * 0.18).clamp(10, 12).toDouble(),
          ),
        ),
      ],
    );
